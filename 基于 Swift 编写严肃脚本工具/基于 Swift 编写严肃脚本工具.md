...
其中前三项其实都是和业务有关， 不同公司的业务技术选项有自己的思考， 我们比较难在短时间内就能推动业务向 Swift 迁移。    但是作为有品味的开发者， 我们还是希望能在日常生活中，更多地使用 Swift， 通过 Swift 强大的类型系统， 安全性，表达性写出更优质的代码。
...
不过今天的主角不是介绍这个跨平台框架。 而是介绍 Swift 也在这个跨平台框架中扮演着重要角色。 那就是代码生成器。
...
大家现在可能还是对这个代码生成器一头雾水，不过可以隐约感觉到，这个代码生成器是比较严肃的，需要投入的时间和精力也比较大。
...
这次我们并没有展示特别复杂的逻辑代码， 但是上面展示的步骤，原则，就能支撑像微信支付跨平台代码生成器这样的命令行工具。   
